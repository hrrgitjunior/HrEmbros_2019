...
export class NavigationService {
  ...
  private handleNavigationChange(route) {
    ...
    if (route.url.value[0].path) {
      ...
      if (i === -1) {
        this.breadcrumbList.push({ name: item, path: ["/wrapper/" + item, this.currentCategory] });
      } else {
  ...
  set currentCategory(newCategory: string) {
    if (newCategory == "")
      ...
      this.router.navigate(['/wrapper/store', newCategory], { relativeTo: this.route });
    ...
    set currentPage(newPage: number) {
        this.router.navigateByUrl(`/wrapper/designs/${this.repo.currentCategory}/${newPage}`);
    }
